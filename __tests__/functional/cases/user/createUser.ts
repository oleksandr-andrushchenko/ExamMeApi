...
describe('Create user', () => {
  test('Unauthorized', async () => {
    const res = await request(framework.app).post('/users')
  ...
  test.each([
    ...
    const res = await request(framework.app).post('/users').send(userCreate).auth(token, { type: 'bearer' })
  ...
  test('Forbidden', async () => {
    ...
    const token = (await framework.auth(user)).token
    const res = await request(framework.app).post('/users').send({
      name: 'any',
      email: 'a@a.com',
      password: '123123',
