...
describe('Query categories', () => {
  test('Empty', async () => {
    ...
    const res = await request(framework.app).post('/').send(categoriesQuery())
  ...
  test.each([
    ...
    const res = await request(framework.app).post('/').send(categoriesQuery(query as CategoryQuerySchema))
  ...
  test('Not empty', async () => {
    ...
    const res = await request(framework.app).post('/').send(categoriesQuery({ size: 50 }, [ 'id', 'name' ]))
