...
describe('Query categories', () => {
  test('Empty', async () => {
    ...
    const res = await request(framework.app).get('/categories')
  ...
  test.each([
    ...
    const res = await request(framework.app).get('/categories').query(query)
  ...
  test.each([
    ...
    const query = { cursor: 'id', size, order: 'asc' }
    const res = await request(framework.app).get('/categories').query({ ...query, ...{ prevCursor } })
    ...
    const lastInStorageId = categories[lastInStoragePosition].id.toString()
    expect(lastInBodyId).toEqual(lastInStorageId)
    const nextCursor = categories[prev - 1].id.toString()
    expect(res.body.meta).toMatchObject({ ...query, ...{ nextCursor } })

    if (firstInStoragePosition > 1) {
      const prevFirstInStorageId = categories[firstInStoragePosition - 1].id.toString()
  ...
  test.each([
    ...
    const query = { cursor: 'id', size, order: 'asc' }
    const res = await request(framework.app).get('/categories').query({ ...query, ...{ nextCursor } })
  ...
  test.each([
    ...
    if (lastInStoragePosition > 1) {
      const prevLastInStorageId = categories[lastInStoragePosition - 1].id.toString()
  ...
  test('Not empty', async () => {
    await framework.clear(Category)
    const categories = await Promise.all([ framework.fixture<Category>(Category), framework.fixture<Category>(Category) ])

    const res = await request(framework.app).get('/categories').query({ size: 50 })
  ...
  test('Empty (GraphQL)', async () => {
    await framework.clear(Category)
    const res = await request(framework.app).post('/graphql').send(categoriesQuery())
  ...
  test.each([
    ...
    const res = await request(framework.app).post('/graphql').send(categoriesQuery(query as CategoryQuerySchema))
  ...
  test.each([
    ...
    { size: 3 },
    { size: 4 },
  ])('Cursor (id, id:asc, size: $size) (GraphQL)', async ({ size }) => {
    await framework.clear(Category)
    const categories = (await Promise.all([
      framework.fixture<Category>(Category),
      framework.fixture<Category>(Category),
  ...
  test.each([
    ...
    { size: 3 },
    { size: 4 },
  ])('Cursor (id, id:desc, size: $size) (GraphQL)', async ({ size }) => {
    await framework.clear(Category)
    const categories = (await Promise.all([
      framework.fixture<Category>(Category),
      framework.fixture<Category>(Category),
  ...
  test.each([
    ...
    { prev: 2, size: 2 },
    { prev: 2, size: 3 },
  ])('Cursor (id, id:asc, prev: $prev, size: $size) (GraphQL)', async ({ prev, size }) => {
    await framework.clear(Category)
    const categories = (await Promise.all([
      framework.fixture<Category>(Category),
      framework.fixture<Category>(Category),
    ...
    const res = await request(framework.app).post('/graphql')
      .send(categoriesQuery({ ...query, ...{ prevCursor } } as CategoryQuerySchema))
  ...
  test.each([
    ...
    { next: 2, size: 2 },
    { next: 2, size: 3 },
  ])('Cursor (id, id:asc, next: $next, size: $size) (GraphQL)', async ({ next, size }) => {
    await framework.clear(Category)
    const categories = (await Promise.all([
      framework.fixture<Category>(Category),
      framework.fixture<Category>(Category),
    ...
    const query = { cursor: 'id', size, order: 'asc' }
    const res = await request(framework.app).post('/graphql')
      .send(categoriesQuery({ ...query, ...{ nextCursor } } as CategoryQuerySchema))
  ...
  test.each([
    ...
    { prev: 2, size: 3 },
    { prev: 2, size: 4 },
  ])('Cursor (id, id:desc, prev: $prev, size: $size) (GraphQL)', async ({ prev, size }) => {
    await framework.clear(Category)
    const categories = (await Promise.all([
      framework.fixture<Category>(Category),
      framework.fixture<Category>(Category),
  ...
  test.each([
    ...
    { next: 2, size: 2 },
    { next: 2, size: 3 },
  ])('Cursor (id, id:desc, next: $next, size: $size) (GraphQL)', async ({ next, size }) => {
    await framework.clear(Category)
    const categories = (await Promise.all([
      framework.fixture<Category>(Category),
      framework.fixture<Category>(Category),
  ...
  test('Not empty (GraphQL)', async () => {
    ...
    const res = await request(framework.app).post('/graphql').send(categoriesQuery({ size: 50 }, [ 'id', 'name' ]))
