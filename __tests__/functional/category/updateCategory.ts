...
describe('Update category', () => {
  test('Unauthorized', async () => {
    ...
    const res = await request(app).patch(`/categories/${ category.id.toString() }`).send({ name: 'any' })

    expect(res.status).toEqual(401)
    expect(res.body).toMatchObject(error('AuthorizationRequiredError'))
  })
  test('Bad request (invalid category id)', async () => {
    const user = await fixture<User>(User, { permissions: [ CategoryPermission.UPDATE ] })
    const token = (await auth(user)).token
    const res = await request(app).patch('/categories/invalid')
  ...
  test.each([
    ...
    const user = await fixture<User>(User, { permissions: [ CategoryPermission.UPDATE ] })
    const token = (await auth(user)).token
    const res = await request(app).patch(`/categories/${ category.id.toString() }`)
